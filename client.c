...
void recv_msg(int sockfd, char *msg) {
  ...
  if (n < 0 || n != 3) error("ERROR reading message from server socket.");
...
int recv_int(int sockfd) {
  ...
  if (n < 0 || n != sizeof(int)) error("ERROR reading int from server socket");
...
void write_server_int(int sockfd, int msg) {
  ...
  if (n < 0) error("ERROR writing int to server socket");
...
void take_turn(int sockfd) {
  ...
  while (1) {
    printf("Enter 0-8 to make a move, or 9 to watch number of active players: ");
...
int main(int argc, char *argv[]) {
  ...
  do {
    recv_msg(sockfd, msg);
    if (!strcmp(msg, "HLD")) printf("Waiting for a second player...\n");
