...
void write_client_int(int cli_sockfd, int msg) {
  ...
  if (n < 0) error("ERROR writing int to client socket");
}
...
int setup_listener(int portno) {
  ...
  if (sockfd < 0) error("ERROR opening listener socket.");
...
void write_client_msg(int cli_sockfd, char *msg) {
  ...
  if (n < 0) error("ERROR writing msg to client socket");
}
...
#ifdef DEBUG printf("[DEBUG] Player %d's move was invalid.\n", player_id);
#endif
...
#ifdef DEBUG printf("[DEBUG] Win by backslash diagonal.\n");
#endif
      return 1;
    }
    if ((last_move == 2 || last_move == 4 || last_move == 6) &&
        (board[1][1] == board[0][2] && board[1][1] == board[2][0])) {
#ifdef DEBUG printf("[DEBUG] Win by frontslash diagonal.\n");
#endif
      return 1;
    }
  }
